...
def _fill_heff_hermitian(heff, xs, ax, xt, axt, dot, omega=None):
    nrow = len(axt)
    row1 = len(ax)
    row0 = row1 - nrow
...
def _fill_heff(heff, xs, ax, xt, axt, dot, omega=None):
    nrow = len(axt)
    row1 = len(ax)
    row0 = row1 - nrow
    for ip, i in enumerate(range(row0, row1)):
        for jp, j in enumerate(range(row0, row1)):
            heff[i,j] = dot(xt[ip].conj(), axt[jp])
            if omega is not None and j==i:
                heff[i,j] = heff[i,j] - omega
...
def davidson_nosym1(aop, x0, precond, tol=1e-12, max_cycle=2, max_space=20,
                    lindep=DAVIDSON_LINDEP, max_memory=MAX_MEMORY,
                    dot=numpy.dot, callback=None,
                    nroots=1, lessio=False, left=False, pick=pick_real_eigs,
                    verbose=logger.WARN, follow_state=FOLLOW_STATE,
...
# but the eigenvectors x0 might not be strictly orthogonal
            xt = None
            x0len = len(x0)
            print ('x0 shape fresh?', x0[0].shape)
            xt, x0 = _qr(x0, dot, lindep)[0], None
            print ('xt shape fresh?', xt[0].shape)
            if len(xt) != x0len:
            ...
            xt = _qr(xt, dot, lindep)[0]
            xt = xt[:40]  # 40 trial vectors at most
            print ('xt shape nofresh?', xt[0].shape)

        axt = aop(xt)
        print ('axt shape',axt[0].shape)
...
def davidson_nosym1_linear_system(aop, x0, propgrd, XiVecs, omega, precond, 
                    tol=1e-12, max_cycle=50, max_space=100,
                    lindep=DAVIDSON_LINDEP, max_memory=MAX_MEMORY,
                    dot=numpy.dot, callback=None,
                    nroots=1, lessio=False, left=False,
                    verbose=logger.WARN, follow_state=FOLLOW_STATE,
                    tol_residual=None, fill_heff=_fill_heff):
    if isinstance(verbose, logger.Logger):
        log = verbose
...
# but the eigenvectors x0 might not be strictly orthogonal
        ...
        fill_heff(heff, xs, ax, xt, axt, dot, omega)
        xt = axt = None
        v = scipy.linalg.solve(heff[:space,:space], bs[:space])
        ...
        xt = [None] * nroots
        if not conv[k]:
                xt[k] = ax0[k] - omega * x0[k] + XiVecs
                dx_norm[k] = numpy.sqrt(dot(xt[k].conj(), xt[k]).real)
                print('norm dx', dx_norm[k])
                conv[k] = dx_norm[k] < toloose
                if conv[k] and not conv_last[k]:
                    # log.debug('root %d converged  |r|= %4.3g  e= %s  max|de|= %4.3g',
                    #           k, dx_norm[k], ek, de[k])
                    conv[k] = True
        ...
        if (not conv[k]) and dx_norm[k]**2 > lindep:
                xt[k] = precond(xt[k], omega-level_shift, x0[k])
        ...
        # log.debug('davidson %d %d  |r|= %4.3g  e= %s  max|de|= %4.3g  lindep= %4.3g',
        #           icyc, space, max_dx_norm, e, norm_min)
        # if len(xt) == 0:
        #     log.debug('Linear dependency in trial subspace. |r| for each state %s',
        #               dx_norm)
        #     conv = [conv[k] or (norm < toloose) for k,norm in enumerate(dx_norm)]
